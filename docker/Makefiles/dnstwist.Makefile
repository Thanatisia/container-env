...
#### Stage 1 (Base)
# IMAGE_NAME ?= thanatisia/lab
# IMAGE_TAG ?= cybersecurity
# BUILD_ARGS ?= --build-arg BASE_IMAGE_NAME="python" --build-arg BASE_IMAGE_TAG="latest" # Set Build Arguments; Format: --build-arg "ARGUMENT=VALUE"
# DOCKERFILE ?= https://raw.githubusercontent.com/elceef/dnstwist/master/webapp/Dockerfile
...
### Containers
CONTAINER_IMAGE_NAME ?= elceef/dnstwist
...
CONTAINER_MOUNT_VOLUMES ?= -v "${HOME}/.config/:${HOME}/.config/" # Mount Host System Volume; -v "[host-system-volume]:[container-volume]:[permissions]"; Other Options: -v "${PWD}/projects:/projects" 
